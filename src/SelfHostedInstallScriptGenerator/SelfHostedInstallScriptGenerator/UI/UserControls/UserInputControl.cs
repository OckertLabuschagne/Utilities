...
namespace SEFI.UserControls
{
    public partial class UserInputControl : UserControl
    {
        ...
        private void BuildUI(InputType value)
        {
            ...
            _InputType = value;
            HasButton = HasButton | _InputType == InputType.Image | _InputType == InputType.Guid | _InputType == InputType.File | _InputType == InputType.Folder;
            if (HasButton)
            {
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            switch(_InputType)
            {
                ...
                case InputType.Image:
                    break;
                case InputType.Folder:
                    FolderBrowserDialog dialog = new FolderBrowserDialog { SelectedPath = _Value as string };
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        (_EditorControl as Label).Text = dialog.SelectedPath;
                        _Value = dialog.SelectedPath;
                ...
                case InputType.File:
                    ...
                    {
                        FileName = _Value as string,
                        Filter = Filter
                    };
                    if (fileDialog.ShowDialog() == DialogResult.OK)
                    {
                        (_EditorControl as Label).Text = fileDialog.FileName;
                        _Value = fileDialog.FileName;
                        OnValueChanged(EventArgs.Empty);
                    }
                    break;
            }
